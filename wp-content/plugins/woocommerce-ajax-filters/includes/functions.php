    ...
    function br_is_term_selected( $term, $checked = FALSE, $child_parent = FALSE, $depth = 0 ) {
        ...
        if ( ! empty($_POST['terms']) and ! empty($term) and is_object( $term ) ) {
            ...
            foreach ( $_POST['terms'] as $p_term ) {
                if ( (  ! empty($p_term[0]) and ! empty($p_term[1]) and $p_term[0] == $term->taxonomy and $term->term_id == $p_term[1] ) or $is_checked ) {
        ...
        if ( ! empty($_POST['price_ranges']) and ! empty($term) and is_object( $term ) and $term->taxonomy == 'price' ) {
            $is_checked = false;
