...
?>
    <div class="wcmp_side_menu">
        <div class="wcmp_top_logo_div"> <img src="<?php echo $vendor->image; ?>" alt="vendordavatar">
            <h3>
                <?php echo get_user_meta(get_current_user_id(), '_vendor_page_title', true) ? get_user_meta(get_current_user_id(), '_vendor_page_title', true) : __('Store Name', 'dc-woocommerce-multi-vendor'); ?>
            </h3>
            <ul>
                <li><a target="_blank" href="<?php echo apply_filters('wcmp_vendor_shop_permalink', $vendor->permalink); ?>"><?php _e('Store', 'dc-woocommerce-multi-vendor'); ?></a> </li>
                ...
                <?php foreach ($nav_items as $key => $item): ?>
                    ...
                    <?php if (current_user_can($item['capability']) || $item['capability'] === true): ?>
                        <li class="<?php if(!empty($item['submenu'])){ echo 'hasmenu';} ?>">
                            ...
                            <a href="<?php echo esc_url($item['url']); ?>" target="<?php echo $item['link_target'] ?>" data-menu_item="<?php echo $key ?>" class="<?php echo implode(' ', array_map('sanitize_html_class', km_wcmp_get_vendor_dashboard_nav_item_css_class($key, $force_active))); ?>">
                                ...
                                <ul class="submenu" <?php if(!in_array('active', wcmp_get_vendor_dashboard_nav_item_css_class($key, $force_active))){ echo 'style="display:none"'; } ?>>
                                    <?php foreach ($item['submenu'] as $submenukey => $submenu): ?>
                                        <?php if(current_user_can($submenu['capability']) || $submenu['capability'] === true): ?>
                                            <li class="<?php echo implode(' ', array_map('sanitize_html_class', km_wcmp_get_vendor_dashboard_nav_item_css_class($submenukey))); ?>">
